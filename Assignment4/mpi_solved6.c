...
******************************************************************************/
// The error is because of changes in reqs and status. The send/irecv is also making use of same
// reqs.
#include "mpi.h"
#include <stdio.h>
#include <stdlib.h>
...
MPI_Request reqs2[REPS*2];
MPI_Status stats2[REPS*2];
...
/* Task 2 does the blocking send operation */
  if (rank == 2) {
    dest = 3;
    for (i=0; i<REPS; i++) {
      MPI_Send(&rank, 1, MPI_INT, dest, i, COMM);
      if ((i+1)%DISP == 0)
...
/* Task 3 does the non-blocking receive operation */
  if (rank == 3) {
    ...
    offset = 0;
    for (i=0; i<REPS; i++) {
      MPI_Irecv(&buf, 1, MPI_INT, src, i, COMM, &reqs2[offset]);
      //MPI_Recv(&buf, 1, MPI_INT, src, tag1, COMM, &stats[offset]);
...
//MPI_Waitall(nreqs, reqs2, stats2);
